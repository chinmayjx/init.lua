...
nnoremap <Leader>P "0P
nnoremap <Leader>fp :CocCommand prettier.forceFormatDocument<CR>
...
nnoremap <M-q> :exec "NERDTreeToggle " . expand('%:p:h')<CR><C-w>=
nnoremap <C-q> :NERDTreeToggle<CR><C-w>=
...
nnoremap <Leader>; :15sp term://bash \| startinsert<CR>
...
nnoremap <Leader>`s :w \| source ~/.config/nvim/init.vim<CR>
...
nnoremap <silent> ,p :call fzf#run({'source': g:CJGetProjects(), 'sink': function('g:CJProjectSelect'), 'options': "", 'window': {'width': 0.8, 'height': 0.4, 'xoffset': 0.5, 'yoffset': 0.2}})<CR>
...
function g:CJUpdateWinBuffs()
	...
	if buf_nm == "NERD_tree_1"
		silent! unlet g:CJWinBuffs[win_id] 
		return
	endif
...
function g:CJClipboardPopup()
  ...
  call add(clip,trim(execute("echo @/")))
  call fzf#run({'source': clip, 'sink': function('g:CJpaste'), 'options': "", 'window':{'width': 0.8, 'height': 15, 'xoffset': 0.5, 'yoffset': 0.2}})
  startinsert
endfunction

nnoremap <silent> <F4>v :call g:CJClipboardPopup()<CR>
inoremap <silent> <F4>v <esc>:let g:CJIsInsert=1 \| call g:CJClipboardPopup()<CR>
