vim.g.mapleader = " "
Map("n", "<leader>sa", "gg0vG$")

Map("n", "<leader>y", "\"+y")
Map("v", "<leader>y", "\"+y")
Map("n", ",,", ",")
Map("n", ",q", ":qa<CR>")
Map("n", ",d", "*Ncgn")
Map("n", ",h", ":%s//\\0/g<left><left><left><left><left>")
Map("v", ",l", ":s//\\0/g<left><left><left><left><left>")
Map("v", ",h", "\"hy:let @h=escape(@h,'\\/')<CR>:%s/\\M<C-r>h/\\0/g<left><left><space><backspace>")
Map("v", ",D", "\"sy:let @/='\\V'.escape(@s, '\\')<CR>cgn")
Map("v", "<leader>d", "y`]p")

Map("n", "t<F4>", "gT")
Map("n", "t<F5>", "<C-w>T")
Map("n", "<F4>c", ":tabnew<CR>")
Map("n", "<F4>x", ":tabc<CR>")

Map("i", "<M-up>", "<C-o>gk", { silent = true })
Map("i", "<M-down>", "<C-o>gj", { silent = true })
Map("n", "<up>", "gk")
Map("n", "<down>", "gj")

Map("n", "<M-S-j>", function() vim.cmd.m("+1") end)
Map("i", "<M-S-j>", "<Esc>:m .+1<CR>i")
Map("v", "<M-S-j>", ":m '>+1<CR>gv")
Map("n", "<M-S-k>", function() vim.cmd.m("-2") end)
Map("i", "<M-S-k>", "<Esc>:m .-2<CR>i")
Map("v", "<M-S-k>", ":m '<-2<CR>gv")

Map("v", "<leader>(", "\"sdi(<C-r>s)")
Map("v", "<leader>[", "\"sdi[<C-r>s]")
Map("v", "<leader>{", "\"sdi{<C-r>s}")

Map("i", "<C-Del>", "<C-o>dw")
Map("i", "<C-h>", "<C-w>")
Map("i", "<C-v>", "<C-r>\"")
Map("i", "<M-z>", "<C-o>u")
Map("i", "<M-Z>", "<C-o><C-r>")
Map("i", "<Home>", "<C-o>^")
Map("i", "<C-s>", "<C-o>:w<CR>")
Map("n", "<C-s>", ":w<CR>")
Map("n", ",x", ":bd<CR>")

Map("n", "<leader>co", ":copen<CR>")
Map("n", "<leader>cn", ":cnext<CR>")
Map("n", "<leader>cp", ":cprev<CR>")

Map("n", "<leader>li", ":lua print(vim.inspect())<left><left>")
Map("n", "<leader>nw", ":setl nowrap<CR>")

Map("n", "<leader>;", ":15sp term://bash | startinsert<CR>")
Map("n", "<F4>v", function()
  SelectFromList(vim.fn.CJClipboardItems(), vim.fn.CJpaste)
  vim.cmd.startinsert()
end)
Map("n", "<leader>ss", ":lua SaveSession()<CR>")
Map("i", "<F4>v", function()
  vim.g.CJIsInsert = 1
  SelectFromList(vim.fn.CJClipboardItems(), vim.fn.CJpaste)
  vim.cmd.startinsert()
end)
SMap("n", "<leader>ca", vim.lsp.buf.code_action)

SMap({ "n", "v" }, "\\y", "\"+y")
SMap({ "n", "v" }, "\\c", "\"_c")
SMap({ "n", "v" }, "\\d", "\"_d")
SMap("n", "\\x", function()
  local cb = vim.fn.winbufnr(0)
  local bi = vim.fn.getbufinfo(cb)[1]
  if bi.changed ~= 0 then
    vim.notify("unsaved changes")
    return
  end
  vim.cmd("b #")
  vim.cmd("bw " .. cb)
end)

vim.cmd [[
  nnoremap <expr> k (v:count > 1 ? "m'" . v:count : '') . 'k'
  nnoremap <expr> j (v:count > 1 ? "m'" . v:count : '') . 'j'
]]
